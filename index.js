...
bot.on("ready", async () => {
    ...
    function setColor()
    {
        ...
        bot.user.setGame(`${users} :סקוויקרים בשרת | Bot by @S4muRaY'#6861`, "http://twitch.tv/s4muray04");
    }
...
bot.on("guildMemberAdd", async member => {
    var role = member.guild.roles.find("name", "סקיוויקרים נודר");
    member.addRole(role);
    var welcomechannel = bot.channels.find("id", "456397510190039051");
    welcomechannel.send("<@" + member.user.id + ">" + " !ברוך הבא לקהילת הציפציפים של ישראל");
    if(member.bot)
    {
